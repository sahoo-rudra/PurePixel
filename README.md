# KLA Semiconductor Image Restoration â€” DegradationAwareNAFNet
...
**DegradationAwareNAFNet**: a FiLM-conditioned NAFNet backbone with PixelShuffle 2Ã— super-resolution head and composite frequency-domain loss, achieving PSNR 24.7450 dB (+1.59 dB over bicubic) on 320 validation images.
...
| **Improvement**| **+1.5915**|**+0.1601**|**âˆ’0.0516**|
...
| H100 est. | 150â€“250 img/s | ~4â€“7 ms | NVIDIA H100 |
...
A lightweight CNN analyses the raw NoisyLR input and produces a 64-dimensional degradation embedding. This embedding is projected into (Î³, Î²) FiLM conditioning pairs for each of the 4 encoder scales. The affine transform is applied as:

```
x = Î³ Â· x + Î² (applied after LayerNorm, before conv)
```

At initialisation Î³ â‰ˆ 1.0 (identity) and Î² â‰ˆ 0.0 (no shift), ensuring a stable training start. This design directly addresses the unknown degradation order constraint â€” the model estimates degradation characteristics without knowing which order speckle, Gaussian noise, and downsampling were applied. An auxiliary output predicts `[speckle_level, gaussian_level, blur_level]` in [0, 1] for interpretability.
...
- **Nonlinearity**: SimpleGate â€” `xâ‚ Â· xâ‚‚` (no ReLU/GELU anywhere)
- **Normalisation**: LayerNorm (not BatchNorm â€” better stability at batch=16)
...
â”œâ”€â”€ train.py
â”œâ”€â”€ inference.py
â”œâ”€â”€ app.py                          # Optional Gradio demo
â”œâ”€â”€ backend.py                      # FastAPI demo backend
â”œâ”€â”€ verify_and_baseline.py
â”œâ”€â”€ configs/
â”‚   â””â”€â”€ nafnet_base.yaml
â”œâ”€â”€ src/
...
â”‚   â””â”€â”€ utils.py
â”œâ”€â”€ weights/
â”‚   â”œâ”€â”€ best_model.pth              # ~390 MB (not in git â€” see below)
â”‚   â””â”€â”€ best_model_246ep_psnr24.74.pth
â”œâ”€â”€ results/
â”‚   â”œâ”€â”€ sample_outputs/
â”‚   â”‚   â”œâ”€â”€ success_cases/           # 002880, 002000
â”‚   â”‚   â””â”€â”€ failure_cases/           # 002881, 002882
â”‚   â””â”€â”€ test_submission_output/      # 400 restored test files
â””â”€â”€ solution_presentation.pptx
```
...
- **FP16 mixed precision** via `torch.amp.autocast` â€” ~30% speedup
- **Gradient accumulation** Ã—2 â€” effective batch size 32 from physical batch 16
- **Synthetic degradation augmentation** â€” 50/50 real/synthetic mixing per step
- **CosineAnnealingWarmRestarts** â€” Tâ‚€=50 epochs, Î·_min=1Ã—10â»â¶
- **Early stopping** â€” patience 30 epochs on val PSNR
- **wandb logging** â€” train/val loss, PSNR, SSIM, LPIPS, visual comparisons, degradation estimate tracking (speckle/gaussian/blur levels)
...
| Run ID | Epochs | Notes |
|-----------|---------|----------------------------|
| `vp58iu9o`| 1â€“200 | Initial training |
| `kbwf67i7`| 201â€“246 | Resumed, found best at 246 |
...
### With Test-Time Augmentation (better quality, 8Ã— slower)
...
Also supports (256, 256) input â†’ (512, 512) output automatically.
...
Last 10% of alphabetically sorted file stems = 320 images (stems: last 320 of 3200). Deterministic, seed=42, identical across all runs. The validation set is used only for PSNR checkpoint selection â€” it is never seen during training.

### Metrics

| Metric | Implementation | Notes |
|--------|---------------|-------|
| PSNR | `10 Ã— logâ‚â‚€(1 / MSE)`, MAX=1.0 | Standard peak signal-to-noise ratio |
...
| Fix1 | FFT=0.35, 80 epochs | 24.3163 dB (âˆ’0.43 dB) | Reverted |
...
5. **Gradient accumulation for effective batch=32 on 8 GB VRAM.** The RTX 4060 Laptop GPU has 8 GB VRAM. Physical batch=16 uses 5057 MB (62% utilisation). Gradient accumulation Ã—2 achieves the stability benefits of batch=32 without exceeding memory limits.
...
- **Use**: Validation metric only â€” NOT used as a training loss
...
- **Use**: Architecture basis â€” NOT used as pretrained weights

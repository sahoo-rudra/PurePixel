...
**DegradationAwareNAFNet**: a FiLM-conditioned NAFNet backbone with PixelShuffle 2x super-resolution head and composite frequency-domain loss, achieving PSNR 24.7450 dB (+1.59 dB over bicubic) on 320 validation images.
...
A lightweight CNN analyses the raw NoisyLR input and produces a 64-dimensional degradation embedding. This embedding is projected into (gamma, beta) FiLM conditioning pairs for each of the 4 encoder scales. The affine transform is applied as:

```
x = gamma * x + beta (applied after LayerNorm, before conv)
```

At initialisation gamma ~= 1.0 (identity) and beta ~= 0.0 (no shift), ensuring a stable training start. This design directly addresses the unknown degradation order constraint -- the model estimates degradation characteristics without knowing which order speckle, Gaussian noise, and downsampling were applied. An auxiliary output predicts `[speckle_level, gaussian_level, blur_level]` in [0, 1] for interpretability.
...
- **Nonlinearity**: SimpleGate -- `x1 * x2` (no ReLU/GELU anywhere)
...
### PixelShuffle 2x SR Head

Converts feature maps from (B, 64, 128, 128) to (B, 1, 256, 256) via sub-pixel convolution. Learned upsampling produces sharper edges than bicubic interpolation, which is critical for semiconductor Manhattan geometry (90-degree lines and corners). The head also handles 256x256 input -> 512x512 output for variable input sizes.

### Composite Loss

```
L_total = 0.6 x Charbonnier + 0.2 x SSIM + 0.2 x FFT
```

| Component | Formula | Purpose |
|-----------|---------|-------|
| Charbonnier | `mean(sqrt(diff^2 + 0.001^2))` | Smooth L1 -- robust to outliers |
...
|   +-- utils.py
|-- weights/
|   |-- best_model.pth              # ~390 MB (GitHub Release v1.0)
...
`best_model.pth` (~390 MB) is stored as a GitHub Release asset due to file size limits.
...
- **Gradient accumulation** x2 -- effective batch size 32 from physical batch 16
- **Synthetic degradation augmentation** -- 50/50 real/synthetic mixing per step
- **CosineAnnealingWarmRestarts** -- T0=50 epochs, eta_min=1e-6
...
### With Test-Time Augmentation (better quality, 8x slower)
...
|--------|---------------|-------|
| PSNR | `10 x log10(1 / MSE)`, MAX=1.0 | Standard peak signal-to-noise ratio |
...
5. **Gradient accumulation for effective batch=32 on 8 GB VRAM.** The RTX 4060 Laptop GPU has 8 GB VRAM. Physical batch=16 uses 5057 MB (62% utilisation). Gradient accumulation x2 achieves the stability benefits of batch=32 without exceeding memory limits.
